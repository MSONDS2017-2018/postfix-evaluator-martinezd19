...
public interface Operator<T> {
  ...
  /**
   * Returns the number of {@link Operand}s that this {@link Operator} needs to perform its
   * operation.
   *
   * @return the number of {@link Operand}s that this {@link Operator} needs to perform its
   *     operation.
   */
  public int getNumberOfArguments();

  /**
   * Returns the precedence of the {@link Operator} for in-fix evaluator
   *
   * @return
   */
  ...
  /**
   * Performs this operation on values supplied via the {@link Operator#setOperand(int, Operand)}
   * method and returns the resulting {@link Operand}.
   *
   ...
   * indexed by 0 so the first {@link Operand} should be set with 0, the second with 1, third with
   * 2, etc...</p>
   *
